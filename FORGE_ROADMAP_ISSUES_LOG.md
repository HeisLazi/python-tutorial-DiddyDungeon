...
## Campaign shell and progression follow-ups — F-257 (2026-09-21)

| ID | Priority | Surface | Finding / requested direction | Planned resolution | Status |
|---|---|---|---|---|---|
| F-257 | P1 | Campaign shell / Forge home | The newly ported Forge home can be displaced by the legacy Tutor Notebook, Infinite Dungeon, and Settings navigation surfaces. The intended destination hierarchy is unclear in the live shell even though the merged Campaign Home/Market/Office is now the source-of-truth experience. | Reconcile the top-level navigation so the new Forge home owns the intended campaign landing slot, while Tutor Notebook, Infinite Dungeon, and Settings remain stable destinations. Preserve the prototype layout and route semantics; do not silently restore the old Character/Homestead pages. | Planned |
| F-258 | P1 | Infinite Dungeon question room | Entering an Infinite Dungeon room can leave the learner without a visible question/challenge prompt, making the next action unclear. | Restore the authored room question and learning context above the IDE, then expose the same guard-break combat vocabulary used by campaign (Guard, Guard Break, failure damage, mechanic state) without replacing the prototype room layout. Add a desktop screenshot check and an interaction test for room entry. | Planned |
| F-259 | P1 | Campaign + Infinite Dungeon combat | The live port has not yet been certified as carrying the prototype combat loop end-to-end. Guard Break should be the offensive objective: breaking guard can stun a mob, suppress its mechanic, and advance a phase rather than simply skipping code validation. | Audit the live submit/run path against the prototype contract first. Then port only the missing guard-break state transitions, phase/weakness presentation, and reward consequences; keep learner correctness as the completion gate. | Planned |
| F-260 | P1 | Shared equipment economy | Campaign and Infinite Dungeon currently risk drifting into separate armor/trinket catalogs and purchase/drop rules. The user wants items found or bought in either mode to be available in the other. | Define one shared authored armor/trinket catalog and explicit ownership bridge. Keep run-local HP/room state separate, but make unlocked armor, trinkets, scrolls, and upgrade metadata persist through the canonical state service with migration and tests. | Planned |
...
Audit snapshot (desktop live route, 2026-09-21): Forge study handoff rendered
two `.terminal-host` PTY surfaces beside Monaco (local shell and AI). Infinite
Dungeon rendered one `dungeon-editor-terminal` self-check region and zero
`.terminal-host` mounts. The latter remains a real port gap for F-262/F-259;
it was not counted as a completed combat or terminal migration.
